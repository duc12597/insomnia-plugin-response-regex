...
module.exports.templateTags = [
  {
    name: 'regex',
    displayName: 'Regex',
    description: "extract a value using regular expression",
    args: [
      {
    ...
    async run(context, field, id, filter, regex, resendBehavior) {
      filter = filter || '';
      ...
      if (field === 'header') {
        ...
        //   attribute = matchJSONPath(body, sanitizedFilter);
        // } else {
        //   attribute = matchXPath(body, sanitizedFilter);
        // }
        attribute = (sanitizedFilter.indexOf('$') === 0)
            ? matchJSONPath(body, sanitizedFilter)
            : matchXPath(body, sanitizedFilter);
        match = attribute.match(regex)
        if (!match) {
          throw new Error(`Pattern not found in "${attribute}".`)
        }
        return match;
      } else {
